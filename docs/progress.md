...
- [ ] Output Types (`brain-design.md` §5.4, Work Breakdown Phase 7) — Research/Forecast/Recommendation structured answers. Not started.
- [ ] Human input workflow (`brain-design.md` Section 11, Phase 13) — still not built, was on the original "explicitly not built yet" list from the very first session and hasn't been revisited since.
- [ ] Engineering agent real specify→implement→execute→verify loop + Model Fitness (Phase 15) — Engineering is still the toy round-half-to-even agent, not wired to `sandbox.py` yet; `serving_model` not yet threaded through the claim pipeline.
- [ ] Content Integrity enforcement tests (Phase 14, §12) — the architecture already prevents ingested/human text from reaching control flow by construction, but this isn't yet backed by explicit tests proving it.
- [ ] Evaluation Infrastructure (Phase 9b) — benchmark harness, adversarial set, calibration dashboard, integrity gates. Not started.
