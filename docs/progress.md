...
Caught one real bug while setting it up: `06-setup-backups.sh`'s one `jq` usage assumed `jq` would be present on the Proxmox host itself — it wasn't (only ever installed on the guests). The rest of the script still completed and produced a working backup, which nearly hid the gap; rewrote that one line as `grep`/`cut` on raw JSON to remove the host-side dependency entirely. `known-bugs.md` entry 19.
