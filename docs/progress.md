...
**Honest note on verification, not glossed over:** this session runs on Windows with no local Python/Linux/root environment, so `pytest -q` could not be run here. The fix was verified by pushing the modified `sandbox.py` directly into the actual target LXC and exercising `run_sandboxed()` and `_cgroup_pids_version()` there against the real cgroups v2 mechanism — real code, real target environment, just not through the pytest harness itself. **Run the actual `pytest -q` suite in a proper Linux/root environment before treating this as fully closed** — expected to still be 109/109 plus the new pure-logic tests, but that has not been executed as a single suite run this session, and should not be assumed.

**Attempted to run the actual full suite inside LXC 104 this session; blocked, not yet resolved.** `pytest` and `pyyaml` aren't in the base Debian 12 template, and the container has no internet egress at all — `ping` to both the LAN gateway (`192.168.0.1`) and `8.8.8.8` are 100% loss from inside the container, even though the container reaches the Proxmox host itself fine and the host reaches the internet fine. This is a *different* symptom from the earlier LAN-reachability gap (this Windows machine → container): that one was host-and-container-both-fine, only a third machine blocked; this one is the container itself unable to leave the host at all, gateway included, which points somewhere else (still not root-caused — worth checking NAT/masquerade rules on the host's `vmbr0` egress path, or whether the earlier per-NIC/cluster firewall changes need to be re-verified now that the MAC is pinned). **Next session: root-cause container internet egress, then `pip install pytest pyyaml` (or `apt install python3-pytest python3-yaml` if apt has a path out), copy the `src/`+`tests/`+`pyproject.toml` tree over (gzipped, ~42KB — small enough to move as a single base64 blob through the host-console paste workflow if no better transfer path exists by then), and run the real `pytest -q` there.** Until that happens, treat the cgroups v2 fix as "verified by direct exercise of the real mechanism" (solid) but not "covered by the full regression suite" (not yet true).
