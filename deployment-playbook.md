...
- Backups: covered separately by `infra/proxmox/06-setup-backups.sh` (daily `vzdump` to `local`, self-pruned), not duplicated here. It protects against a deleted/corrupted guest on an otherwise-healthy host — it does **not** protect against losing the whole physical host, since the backups live on that same machine. See `infra/proxmox/README.md` for the honest scope of what it does and doesn't cover.
