...
### 24. The OLMo 3 7B model-lab guest swap-thrashes after hours of use — ~45× slower, not down
...
**Fix:** **Not applied — needs the owner** (changing a guest is outside what automated sessions may do). Options, least to most invasive: restart `llama-server` on 116 (clears it temporarily); run it with `--mlock` or `--no-mmap` so weights stay resident; give the guest more RAM (there is ample headroom under the 80% cap). Worth checking whether `mistral-7b` (VMID 115, same 10 GB shape) shows the same creep.
**Workaround for development:** `ATHENAEUM_OFFLINE_MODELS=1` (`tests/conftest.py`) makes every agent's model fallback behave as "backend unreachable → no claim", so non-model changes can still be fully verified; run with `--ignore=tests/test_model_backed_reasoning.py`. The other live-model tests fail cleanly with `BackendUnavailable ... timed out`, which is itself the confirmation of this cause.
**Lesson:** "healthy" (`/health` ok) is not "usable" — a health check that doesn't measure latency can't see thrashing. And a full-suite slowdown with an idle host means look *inside* the slow dependency for memory pressure before suspecting the code under test.
