...
**What happened:** `infra/proxmox/06-setup-backups.sh`'s first command, `pvesh get /storage/local --output-format json | jq -r '.content'`, failed with `jq: command not found` when actually run on `proxmox01`. The rest of the script (systemd unit install, timer, first backup run) still completed successfully despite this, which made it easy to nearly miss — the failure was silent enough that the overall result (a working backup, real archives on disk) looked like full success.
**Root cause:** `jq` had been explicitly `apt install`ed on the two LXC guests (104, 106) earlier in the session, and that created an unstated assumption that it was "just available" everywhere in this project's tooling — but the Proxmox **host** itself is a separate machine with its own package set, never verified to include `jq` at all.
**Fix:** Rewrote the one `jq` usage in `06-setup-backups.sh` as `grep -o '"content":"[^"]*"' | cut -d'"' -f4` on the raw JSON instead — uglier, but zero extra dependencies for a script whose whole point is "just works on any fresh host." Any *future* host-only script in `infra/proxmox/` should make the same choice: guest-side tooling scripts can assume `jq`/`curl` (this project installs them there deliberately), host-only scripts (`00-`, `04-`, `06-`) should not assume anything beyond what a stock Proxmox install ships with, unless the script installs its own dependency first and checks that install succeeded.
**Lesson:** "It mostly worked" is not the same as "it worked as designed" — a script that silently skips one step and still produces a plausible-looking result (real backup files existed) can hide a real gap (was the storage's content-type actually set correctly, or did it already happen to be right?) that only surfaces later, at exactly the moment you're relying on it. Check the *specific* step that failed, not just whether the overall outcome looks fine.
